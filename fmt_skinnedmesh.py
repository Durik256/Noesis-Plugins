...
def registerNoesisTypes():
   handle = noesis.register("Battlefield 2 (2005)", ".skinnedmesh")
...
class Lod:
    def __init__(self, bs):
        ...
        self.numRigs = bs.readUInt()
        self.rigs = [Rig(bs) for x in range(self.numRigs)]

class Rig:
    def __init__(self, bs):
        self.numBones = bs.readUInt()
        self.bones = [Bone(bs) for x in range(self.numBones)]

class Bone:
    def __init__(self, bs):
        self.id = bs.readUInt()
        self.mat = NoeMat44.fromBytes(bs.readBytes(64)).toMat43()
    
class Material:
    def __init__(self, bs):
        self.shaderFile = noeAsciiFromBytes(bs.readBytes(bs.readUInt()))
        technique = noeAsciiFromBytes(bs.readBytes(bs.readUInt()))
